...
Description: Raw iLink Bot API HTTP calls
...
Details:
---------
ILinkApi — low-level HTTP client for the iLink Bot API. Maps 1:1 to REST endpoints: get_qr_code for login QR, poll_qr_status
for scanning state, get_updates for long-poll message retrieval, send_message for text replies, send_typing for typing
indicators, get_upload_url for CDN upload authorization, upload_to_cdn for raw byte upload, notify_start/notify_stop for
connection lifecycle. Also provides static builders: build_text_message() and build_media_message() to construct wire-format
message dicts. All POST calls carry auth headers (Bearer token, iLink-App-Id, iLink-App-ClientVersion) and a base_info block.
"""
