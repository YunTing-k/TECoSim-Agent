...
Description: WeChat iLink Bot client — lifecycle, messaging, and media orchestration
...
Details:
---------
WeChatBot orchestrator — the single user-facing class. Manages the bot lifecycle (login, long-poll start/stop), message
dispatching (on_message handlers), sending text and media (reply, reply_media), downloading media from incoming messages
(download, download_raw), uploading to CDN (upload), and typing indicators (send_typing, stop_typing). Internally delegates
to auth.py for login, protocol.py for HTTP calls, and crypto.py for AES-128-ECB encrypt/decrypt of CDN media. Maintains
an in-memory _context_tokens dict (user_id -> context_token) populated from incoming messages and required for all outgoing replies.
"""
...
from .auth import clear_credentials, login
from .crypto import (
    decode_aes_key,
    decrypt_aes_ecb,
