...
Details:
---------
Custom color-logger wrapper using a shared 'logger' singleton (composition + property delegation) to avoid duplicate handlers
across imports. Adds a custom `PARAM` log level (25). Supports console output (dev mode) and file output with colorlog formatting.
Monkey-patches `param()` onto standard `logging.Logger` for universal access.
"""
import os
import logging
import datetime
