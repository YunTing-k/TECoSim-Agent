...
Description: IR Drop simulation agent neural network pipeline realization
...
Details:
Training pipeline of the Unet and DDIM model.
------------------------------------------------------------------------------------------------------------------------
...
def unet_train(unet: nn.Module, path: str, hdf5_config: dict[str, Union[str, int, float, bool]], train_batch: int,
...
def unetland_train(unetland: nn.Module, path: str, hdf5_config: dict[str, Union[str, int, float, bool]], train_batch: int,
                   epoch=60, device='cuda:0', accumulation_steps=1, epoch_save=True, update_count=20):
    """Train the UnetLand"""
    """get the dataset and data loader"""
    train_set = nn_utils.get_dataset(path=path, hdf5_config=hdf5_config)
    ...
    """loss function and optimizer"""
    criterion = nn_utils.get_loss_function('MSELoss')  # loss function
    optimizer = nn_utils.get_optimizer(unetland.parameters(), lr=5e-4, weight_decay=1e-3)  # optimizer
    total_steps = (len(train_loader) // accumulation_steps) * epoch
    scheduler = CosineAnnealingLR(
        optimizer,
        T_max=total_steps,
    ...
    unetland.initialize()  # initialize nn
    unetland.train()  # toggle train mode

    sys_log.info('UnetLand training start')
    if len(train_loader) % accumulation_steps != 0:
        sys_log.warning("Mismatched accumulation steps %d with train loader length %d",
                        accumulation_steps, len(train_loader))
    for epoch in range(epoch):  # train
        ...
        optimizer.zero_grad()
        process_bar = tqdm(enumerate(train_loader), total=len(train_loader),
                           desc='UnetLand Training', leave=True, unit='batch')
        for i, (samples, v_real) in process_bar:
            v_real_min = v_real.amin(dim=[2, 3], keepdim=True)  # get the input voltage's min value
            v_real_rel = (v_real - v_real_min) / (1.0 - v_real_min)  # get the case-wise normalization
            v_real_rel = v_real_rel.to(device)
            samples = samples.to(device)
            v_pred_rel = unetland(samples)
            ...
            grad_loss_x = nnf.smooth_l1_loss(pred_grad_x, real_grad_x, reduction='mean')
            grad_loss_y = nnf.smooth_l1_loss(pred_grad_y, real_grad_y, reduction='mean')
            loss2 = grad_loss_x + grad_loss_y  # get gradient's loss
            loss = 0.7 * loss1 + 30.0 * loss2
            sys_log.info("loss1: %.4e, loss2: %.4e", loss1.detach(), loss2.detach())
            loss = loss / accumulation_steps  # scale to mean
            loss.backward()  # backward propagation
            if ((i + 1) % accumulation_steps == 0) or (i + 1 == len(train_loader)):
                optimizer.step()  # update weight
        ...
        process_bar.close()
        if epoch_save:
            torch.save(unetland.state_dict(), '../model/unetland' + str(epoch) + '.pth')
    """save results"""
    nn_utils.numpy_to_mat(train_loss_array, '../model/unetland_train_loss.mat')
    sys_log.info('UnetLand training done')
