...
Details:
---------
Web support with two capabilities: (1) Web fetch - validates URL (DNS, private network check), fetches via httpx with
redirect handling, converts HTML to Markdown (trafilatura), caches results, processes via fast LLM. (2) Web search -
multi-backend support (Exa, Tavily, Linkup, DDGS) with configurable domains, results summarized by LLM into Markdown with
source hyperlinks.
"""
import httpx
import socket
