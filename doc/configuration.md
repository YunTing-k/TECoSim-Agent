...
| `WEB_FETCH_ENABLE_HTTP2` | 是否启用 HTTP/2 协议（`true`/`false`）/ Whether to enable HTTP/2 protocol                                                                                                                                                                                         |
| `WEB_SEARCH_BACKEND` | 网络搜索后端，可选：[`Exa`](https://exa.ai/)、[`Tavily`](https://www.tavily.com/)、[`Linkup`](https://www.linkup.so/)、`DDGS`（除了`DDGS`都需要注册账号获取API Key） / Web search backend (Except for `DDGS`, all of them require registering an account and obtaining an API Key.) |
...
| `RESUME_RENDER_USER_HISTORY_AS_MD` | 恢复会话时是否以 Markdown 格式渲染用户历史消息 / Whether to render user history messages as Markdown when resuming session                                                                                                                                                  |
...
> - `DDGS`（默认）：无需 API Key，开箱即用。其他后端失败时自动回退到 `DDGS` / No API key required, works out of the box; auto-fallback to `DDGS` when other backends fail
> - [`Exa`](https://exa.ai/)、[`Tavily`](https://www.tavily.com/)、[`Linkup`](https://www.linkup.so/)：需注册账号获取 API Key 后填入 `WEB_SEARCH_API_KEY` / Requires account registration and API key in `WEB_SEARCH_API_KEY`
...
> 
> WeChat 模式下额外启用两个专用工具：`wechat_status`（查看机器人连接状态、消息队列、CDN 缓存统计）和 `wechat_send_file`（向用户发送图片/视频/文件）。终端模式的 `ask_user_question` 交互式提问工具在 WeChat 模式下不可用。
> Two additional tools are enabled in WeChat mode: `wechat_status` (query bot connection state, message queue, CDN cache stats) and `wechat_send_file` (send image/video/file to user). The interactive `ask_user_question` tool is unavailable in WeChat mode.
> 
...
> 
> WeChat 消息在终端和 LLM 上下文中通过 XML 标签包裹展示（`WECHAT_PROMPT_START_LABEL` / `WECHAT_PROMPT_END_LABEL`）。退出时 Agent 向绑定用户发送告别或错误通知（`WECHAT_BOT_NORMAL_EXIT_LIST` / `WECHAT_BOT_ERROR_EXIT_LIST`），随后保存 CDN 缓存和消息历史。更多实现细节（锁定/拒绝回复文案列表、媒体下载流程等）参见 [constants_reference.md](./constants_reference.md) 和 [wechat_behavior.md](./wechat_behavior.md)。
>
> WeChat messages are wrapped in XML tags in the terminal and LLM context (`WECHAT_PROMPT_START_LABEL` / `WECHAT_PROMPT_END_LABEL`). On exit the agent sends a farewell or error notification to the bound user (`WECHAT_BOT_NORMAL_EXIT_LIST` / `WECHAT_BOT_ERROR_EXIT_LIST`), then saves CDN cache and message history. See [constants_reference.md](./constants_reference.md) and [wechat_behavior.md](./wechat_behavior.md) for more implementation details.
